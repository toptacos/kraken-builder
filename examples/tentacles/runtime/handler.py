...
def _plan(p: dict) -> dict:
    ...
    network = str(p.get("network") or net.get("name") or "kraken_dev")
    ...
    use_ssh = bool(ssh_key) and bool(
        p.get("use_ssh") or (services.get("ssh") or {}).get("use")
    )
    defaults = {
        "data": str(
            Path(os.environ.get("KRAKEN_HOME", str(Path.home()))) / ".kraken" / "data"
        ),
...
def main() -> int:
    ...
    if action == "build":
        ...
        if not df.exists():
            return _ok(
                req, {"error": f"missing Dockerfile {df}", "plan": plan}, ok=False
            )
        exe = _docker()
        if not exe:
            return _ok(
        ...
        proc = subprocess.run(
            [exe, "build", "-t", plan["image"], "-f", str(df), str(df.parent)],
            capture_output=True,
            text=True,
        )
        return _ok(
        ...
        body = json.dumps(
            {
                "v": 1,
                "op": "invoke",
                "action": p.get("target_action") or "hello",
                "payload": p.get("inner") or {},
            }
        )
        if not exe:
            return _ok(
        ...
        if proc.returncode == 125:
            return _ok(
                req,
                {
                    "mode": "dry-run",
                    "code": 125,
                    "hint": "image missing or docker daemon refused; argv recorded",
                    "argv": plan["argv"],
                    "stdin": body,
    ...
    if action == "publish":
        ...
        dest = (
            (p.get("services") or {}).get("upload", {}).get("uri")
            or plan["services"].get("upload")
            or "local://store"
        )
        return _ok(
            req,
            {
                "published": dest,
                "hint": "kraken run store put --payload path=… backend=s3|local",
                "plan": plan,
            },
        )
